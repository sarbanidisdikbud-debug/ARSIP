        ...
        <aside className={`${isSidebarOpen ? 'w-64' : 'w-20'} transition-all duration-300 bg-slate-900 text-white flex flex-col z-50`}>
          <div className="p-6 flex items-center justify-between">
            <div className={`flex items-center gap-3 overflow-hidden ${!isSidebarOpen && 'justify-center'}`}>
              ...
              {isSidebarOpen && <span className="font-bold text-xl tracking-tight text-white">ArsipSurat</span>}
            </div>
          </div>
        ...
        <main className="flex-1 overflow-y-auto bg-slate-50">
          <header className="sticky top-0 bg-white/80 backdrop-blur-md border-b border-slate-200 z-40 px-8 py-4 flex items-center justify-between">
            <div className="flex items-center gap-4">
              <h1 className="text-xl font-bold text-slate-800 uppercase tracking-tight">Sistem Arsip Digital</h1>
            </div>
            <div className="flex items-center gap-4">
          ...
          <div className="p-8">
            <Routes>
              ...
              <Route 
                path="/add" 
                element={isAdmin ? <LetterForm onAdd={addLetter} /> : <Navigate to="/" replace />} 
              />
              <Route 
                path="/users" 
                element={isAdmin ? <UserManagement users={users} onAddUser={addUser} onDeleteUser={deleteUser} /> : <Navigate to="/" replace />} 
              />
              <Route 
                path="/letter/:id" 
                element={<LetterDetail letters={letters} userRole={user.role} onUpdate={updateLetter} onDelete={deleteLetter} />} 
              />
              <Route path="/settings" element={<div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-100">Fitur Pengaturan Sedang Dikembangkan</div>} />
